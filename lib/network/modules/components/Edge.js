  ...
  drawLabel(ctx, viaNode) {
    if (this.options.label !== undefined) {
      ...
      if (node1.id != node2.id) {
        ...
        let lSize = this.labelModule.getSize();

        let draw_x = this.body.view.translation.x + lSize.left*scale;
        let draw_y = this.body.view.translation.y + lSize.top*scale;

        let left_x = draw_x;
        let top_y = draw_y;
        let right_x = left_x + lSize.width*scale;
        let bottom_y = top_y + lSize.height*scale;
